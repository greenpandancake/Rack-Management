...
export function App() {
  ...
  if (auth.loading) return <div className="min-h-full grid place-items-center text-sm text-slate-600">Loading...</div>;
      ...
      <header className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
          <Link to="/" className="flex items-center gap-3">
            <span className="grid place-items-center h-9 w-9 rounded-lg bg-slate-900 text-white font-bold">M</span>
            <span>
              <span className="block font-bold leading-tight">MPL Smart Rack</span>
              <span className="block text-xs text-slate-500 leading-tight">Warehouse rack control</span>
            </span>
          </Link>
          <nav className="flex gap-1 ml-auto text-sm items-center">
            ...
            <button onClick={auth.logout} className="px-3 py-2 rounded-md text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition">
              Sign out
            </button>
          </nav>
        </div>
