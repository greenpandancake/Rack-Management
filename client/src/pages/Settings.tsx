    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="app-panel p-6 space-y-4">
        ...
        <form onSubmit={handleCreateUser} className="grid md:grid-cols-6 gap-2 items-end">
          ...
          <label className="block md:col-span-1">
            <span className="block text-xs font-semibold text-slate-600 mb-1">Name</span>
          ...
          <label className="block md:col-span-1">
            <span className="block text-xs font-semibold text-slate-600 mb-1">Telegram</span>
          ...
          <button disabled={busy} className="bg-slate-900 text-white rounded px-3 py-1.5 text-sm disabled:opacity-50">Add user</button>
        </form>
        {userMsg && <div className="text-sm text-slate-700">{userMsg}</div>}
            ...
            <tbody className="divide-y">
              {(users?.users ?? []).map((u) => (
                <tr key={u.id}>
                  ...
                  <td className="py-2 pr-3">
                    <button disabled={busy} onClick={() => patchUser(u, { isActive: !u.isActive })} className={`rounded px-2 py-1 text-xs ${u.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                      {u.isActive ? 'Active' : 'Disabled'}
                    </button>
                  </td>
                  <td className="py-2 pr-3 text-xs text-slate-500">{u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleString() : '-'}</td>
                  <td className="py-2 pr-3 text-right">
                    <button disabled={busy} onClick={() => resetPassword(u)} className="border rounded px-2 py-1 text-xs hover:bg-slate-100">Reset password</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <form onSubmit={handleSubmit} className="app-panel p-6 space-y-4">
        ...
        <label className="block">
          <span className="block text-xs font-semibold text-slate-600 mb-1">Rows (comma separated, e.g. A,B,C,D,E)</span>
          <input className="border rounded px-2 py-1 w-full text-sm" value={rowsCsv} onChange={(e) => setRowsCsv(e.target.value)} />
        </label>
        <div className="grid grid-cols-2 gap-4">
          <label className="block">
            <span className="block text-xs font-semibold text-slate-600 mb-1">Levels</span>
          ...
          <label className="block">
            <span className="block text-xs font-semibold text-slate-600 mb-1">Slots per Level</span>
        ...
        <button disabled={busy} className="bg-slate-900 text-white rounded px-4 py-2 text-sm disabled:opacity-50">
          {busy ? 'Saving…' : 'Save Settings'}
        </button>
        {msg && <div className="text-sm text-slate-700">{msg}</div>}
      </form>
      ) : (
        <div className="app-panel p-6 text-sm text-slate-600">
      ...
      {isSuperAdmin && <div className="app-panel p-6 space-y-4">
        ...
        {grouped.length === 0 && <div className="text-sm text-slate-500">No slots yet.</div>}
        {grouped.map(([row, rowSlots]) => {
          const rowInUse = rowSlots.some((s) => activeSlotOccupants(s).length > 0);
          return (
          <div key={row} className="space-y-1">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold text-slate-500">Row {row}</div>
              <button
                disabled={busy || rowInUse}
                onClick={() => handleDeleteRow(row)}
                title={rowInUse ? 'Clear all cargo in this row first' : `Delete row ${row}`}
                className="text-[11px] border border-red-300 text-red-600 rounded px-2 py-0.5 hover:bg-red-50 disabled:opacity-40 disabled:hover:bg-transparent"
              >
                Delete row {row}
              </button>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`border rounded p-2 text-xs space-y-1 ${
                      s.isActive ? 'bg-white' : 'bg-slate-100 border-dashed'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-semibold">{s.id}</span>
                      <span className={inUse ? 'text-emerald-600' : 'text-slate-400'}>
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        className="flex-1 border rounded px-1.5 py-0.5 text-[11px] hover:bg-slate-100 disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 border border-red-300 text-red-600 rounded px-1.5 py-0.5 text-[11px] hover:bg-red-50 disabled:opacity-40 disabled:hover:bg-transparent"
                      >
                        Delete
                      </button>
                    </div>
